...
LearningX Alert는 숭실대학교 SSU LearningX/Canvas의 과제, 퀴즈, 토론, 페이지 등 학습 일정의 마감 정보를 가져와 앱에서 확인하고 로컬 알림을 예약하는 Flutter 앱입니다.

사용자는 앱에서 SSU Canvas 로그인을 진행하고, Canvas access token을 생성 및 저장한 뒤 앞으로 60일 이내의 마감 일정을 동기화할 수 있습니다. 동기화된 일정은 기기 로컬 캐시에 저장되며, 설정한 알림 시점에 맞춰 마감 알림을 받을 수 있습니다.
...
- `apps/learningx_alert`: 로그인, 일정 목록 표시, 설정, 로컬 알림 예약을 담당하는 Flutter 앱입니다.
- `packages/learningx_api`: Canvas API access token을 사용해 사용자 정보와 planner item을 가져오고 앱에서 사용하는 `LearningItem` 모델로 변환합니다.

## 본인이 구현한 부분

- Flutter 앱 기본 화면 구성 및 라우팅
- SSU Canvas 로그인 WebView 화면 구현
- Canvas 세션 확인 및 access token 자동 생성 로직 구현
- access token 보안 저장 및 로그아웃 처리 구현
- Canvas Planner API 연동용 `learningx_api` 패키지 구현
- API 응답을 앱 전용 `LearningItem` 모델로 변환하는 로직 구현
- 홈 화면의 일정 목록, 동기화 상태, 오류 처리 UI 구현
- `SharedPreferences` 기반 일정 캐시 및 마지막 동기화 시각 저장 구현
- `flutter_local_notifications` 기반 마감 알림 예약 구현
- 알림 설정 화면과 알림 재예약 기능 구현
- Melos 기반 모노레포 스크립트 구성
- API 클라이언트 단위 테스트 및 실제 API 확인용 live test 작성
